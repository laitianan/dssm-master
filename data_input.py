...
def get_lcqmc():
    ...
    # train_set = trans_lcqmc(train_examples())
...
def train_examples(file="./data/train_merge.csv",sep="\t"):
    ...
    with open(file,"r",encoding="utf8") as f:
        ...
        for line in lines:
        	line=line.strip("\n").split(sep)
        	res.append(RowObj(line[0],line[1],line[2]))
        # lines=[RowObj(e.split("\t")[0],e.split("\t")[1],e.split("\t")[2]) for e in lines]
        return res
...
def get_test_bert_by_arr(test_arr:list, vocab:Vocabulary, is_merge=0):
    ...
    for line in test_arr:
        if len(line) == 2:
            ...
            t1, t2 = line   # [t1_ids, t1_len, t2_ids, t2_len, label]
        if len(line) == 3:
            label,t1, t2 = line   # [t1_ids, t1_len, t2_ids, t2_len, label]
        if is_merge:
            out_ids1, mask_ids1, seg_ids1, seq_len1 = vocab._transform_2seq2bert_id(t1, t2, padding=1)
